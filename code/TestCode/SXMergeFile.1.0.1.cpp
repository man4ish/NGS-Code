...
void manual() 
{
     ...
     cout<< "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n";
     cout<< "Usage:\n\n";
     cout<< "\tSXMergeFile <Normal Sample> <Cancerous Sample> \n\n";
}
...
void getcommon_snps(char* filename1,char* filename2) 
{
     ...
     uniquefile1 <<"Chr\tOffset\tAllele\tCG1.SNP_Density\tCG2.SNP_Density\tTotal SNP Desnity\tCG1.Read_Density\tCG2.Read_Density\tTotal_Read_Desity\tCG1.AVQS\tCG2.AVQS\tAVQS\tdbSNP\tRS\n";
     common <<"Chr\tOffset\tAllele\tCG1.SNP_Density\tCG2.SNP_Density\tTotal SNP Desnity\tCG1.Read_Density\tCG2.Read_Density\tTotal_Read_Desity\tCG1.AVQS\tCG2.AVQS\tAVQS\tdbSNP\tRS\n";
...
common << records1[0]<<"\t"<<records1[1]<<"\t"<<records1[2]<<"\t"<<records1[3]<<"\t"<<records2[3]<<"\t"<<TotalSd<<"\t"<<records1[4]<<"\t"<< records2[4]<<"\t"<<TotalRd<<"\t"<<avqs1<<"\t"<<avqs2<<"\t"<<avqs<<"\t"<<records1[6]<<"\t"<<rs<< endl;
                  commcount++;
              }
              else 
              {
                 ...
                 uniquefile1 << records1[0]<<"\t"<<records1[1]<<"\t"<<records1[2]<<"\t"<<records1[3]<<"\t-\t"<<records1[3]<<"\t"<<records1[4]<<"\t-\t"<<records1[4]<<"\t"<<records1[5]<<"\t-\t"<<records1[5]<<"\t"<<records1[6]<<"\t"<<rs_s1<<endl;
                 fcount2++;
              } 
           }
         }
...
{   
     ...
     uniquefile2 <<"Chr\tOffset\tAllele\tCG1.SNP_Density\tCG2.SNP_Density\tTotal_SNP_Desnity\tCG1.Read_Density\tCG2.Read_Density\tTotal_Read_Desity\tCG1.AVQS\tCG2.AVQS\tAVQS\tdbSNP\tRS\n";
     for( m::iterator ii=m1.begin(); ii!=m1.end(); ++ii) 
     {
          ...
          if(it == m3.end()) 
          {
             ...
             uniquefile2 << records[0]<<"\t"<< records[1]<<"\t"<< records[2]<<"\t-\t"<<records[3]<<"\t"<<records[3]<<"\t-\t"<< records[4]<<"\t"<<records[4]<<"\t-\t"<<records[5]<<"\t"<<records[5]<<"\t"<<records[6]<<"\t"<<rs_s2<<endl;
          }
     }
     uniquefile2.close();
}
